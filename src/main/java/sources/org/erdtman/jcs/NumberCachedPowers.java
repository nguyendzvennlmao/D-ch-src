...
class NumberCachedPowers {
    ...
    public static int getCachedPower(int i, int i2, int i3, NumberDiyFp numberDiyFp) {
        CachedPower cachedPower = CACHED_POWERS[(((GRISU_CACHE_OFFSET + ((int) Math.ceil((((i2 - i) + 64) - 1) * kD_1_LOG2_10))) - 1) / 8) + 1];
        numberDiyFp.setF(cachedPower.significand);
        numberDiyFp.setE(cachedPower.binaryExponent);
        if ($assertionsDisabled || (i2 <= numberDiyFp.e() + i && numberDiyFp.e() + i <= i3)) {
